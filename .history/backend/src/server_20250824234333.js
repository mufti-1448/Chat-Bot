...
//         console.log("🤖 Pertanyaan user:", userMessage);

//         // ⚠️ NON-AKTIFKAN BUILD CONTEXT (TIDAK PAKAI DATABASE)
//         // const context = await buildContext();
//         const context = {}; // ← PAKAI OBJECT KOSONG

//         // ✅ LANGSUNG GENERATE RESPONSE TANPA DATABASE
...
// ===== GEMINI SETUP =====
const GEMINI_API_KEY = process.env.GEMINI_API_KEY || "AIzaSyD2etqul1@Rk5JHCU3Qr3DISvXAP5KHa-0";
...
async function generateResponse(userMessage) {
    try {
        ...
        return response.text() || "Maaf, tidak bisa menjawab.";
    } catch (error) {
        ...
        // Cek jika API key invalid
        if (error.message.includes("API key") || error.message.includes("key")) {
            console.error("❌ Mungkin API key tidak valid!");
        }
...
app.post("/api/ask", async (req, res) => {
    try {
        ...
        console.log("🤖 Pertanyaan:", userMessage);

        // ✅ Tambahkan try-catch khusus untuk generateResponse
        let response;
        try {
        ...
        console.error("❌ General error in /api/ask:", error);
