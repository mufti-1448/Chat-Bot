...
const fetch = require("node-fetch"); // Tambahkan ini di bagian atas
...
const schoolKeywords = [
    ...
    "paskibra", "pmr", "pramuka", "website", "jadwal", "kegiatan", "perpustakaan", "kontak", "berita"
];
...
function isGeneralEducation(q) {
    const t = q.toLowerCase();
    return ["kurikulum", "p5", "magang", "prakerin", "merdeka belajar", "smk vs sma", "kejuruan", "kompetensi"]
        .some(k => t.includes(k));
}
...
app.post("/ask", async (req, res) => {
    ...
    if (!q) return res.json({ answer: "Masukkan pertanyaan." });

    try {
        if (isSchoolQuestion(q)) {
            // Coba jawab dari DB (rule sederhana)
            ...
            // Jika tidak ketemu rule-nya -> minta Gemini dengan konteks DB
        ...
        console.error(e);
        return res.json({ answer: "Terjadi kesalahan di server." });
    }
});

// ====== Endpoint refresh scraping (opsional, pakai token sederhana) ======
