...
async function buildContext() {
    try {
        const [statis, jur, eks, news] = await Promise.all([
            db.all(`SELECT key, value FROM statis`), // ✅ Updated
            db.all(`SELECT nama, deskripsi FROM jurusan`), // ✅ Updated
            db.all(`SELECT nama, pembina, deskripsi FROM ekskul`), // ✅ Updated
            db.all(`SELECT title, link, date FROM berita ORDER BY id DESC LIMIT 5`) // ✅ Updated
        ]);
    ...
    try {
        ...
        if (isSchoolQuestion(q)) {
            ...
            if (q.toLowerCase().includes("jurusan")) {
                const rows = await db.all(`SELECT nama, deskripsi FROM jurusan`); // ✅ Updated
                if (rows.length) {
            ...
            if (q.toLowerCase().includes("ekstrakurikuler") || q.toLowerCase().includes("ekskul")) {
                const rows = await db.all(`SELECT nama, pembina, deskripsi FROM ekskul`); // ✅ Updated
                if (rows.length) {
            ...
            if (q.toLowerCase().includes("berita") || q.toLowerCase().includes("info terbaru")) {
                const rows = await db.all(`SELECT title, link, date FROM berita ORDER BY id DESC LIMIT 5`); // ✅ Updated
                if (rows.length) {
            ...
            if (q.toLowerCase().includes("ppdb") || q.toLowerCase().includes("pendaftaran")) {
                const ppdbInfo = await db.all(`SELECT value FROM statis WHERE key = 'ppdb_info'`); // ✅ Updated
            ...
            if (q.toLowerCase().includes("kontak") || q.toLowerCase().includes("alamat") || q.toLowerCase().includes("telpon")) {
                const rows = await db.all(`SELECT key, value FROM statis WHERE key IN ('alamat', 'telp', 'email')`); // ✅ Updated
...
app.get("/health", async (req, res) => {
    try {
        const health = await db.healthCheck(); // ✅ Updated health check
        res.json({
