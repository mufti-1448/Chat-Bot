...
// [IN server.js, MODIFY THE /ask ENDPOINT TO RETURN QUICK REPLIES TOO]
...
app.post("/ask", async (req, res) => {
    ...
    try {
        let quickReplies = []; // Tambahkan quick replies dinamis
        ...
        if (isSchoolQuestion(q)) {
            ...
            //     }
            // }
            // [IN server.js - FIND THE JURUSAN SECTION AND REPLACE WITH THIS]

            if (q.toLowerCase().includes("jurusan")) {
                const rows = await all(`SELECT nama, deskripsi FROM jurusan`);
                if (rows.length) {
                    // Format jawaban yang lebih informatif
                    let jawaban = "🏫 **Jurusan yang tersedia di SMK Syafi'i Akrom:**\n\n";

                    rows.forEach((jurusan, index) => {
                        jawaban += `**${index + 1}. ${jurusan.nama}**\n`;
                        jawaban += `   ${jurusan.deskripsi || 'Deskripsi belum tersedia'}\n\n`;
                    });

                    jawaban += "ℹ️ Untuk informasi detail setiap jurusan, kunjungi: https://ponpes-smksa.sch.id/jurusan/";

                    // Quick replies untuk setiap jurusan
                    const quickReplies = rows.map(r => `Info ${r.nama}`).slice(0, 3);
            ...
            if (q.toLowerCase().includes("ekstrakurikuler") || q.toLowerCase().includes("ekskul")) {
                const rows = await all(`SELECT nama FROM ekskul`);
                if (rows.length) {
                    quickReplies = rows.map(r => `Ekskul ${r.nama}`).slice(0, 3);
                    return res.json({
                        answer: "Ekstrakurikuler:\n- " + rows.map(r => r.nama).join("\n- "),
                        quickReplies: quickReplies
                    });
            ...
            // [ADD SIMILAR quickReplies FOR OTHER CASES...]

            const ctx = await buildContext();
            const answer = await askGemini(q, ctx);
            return res.json({
                answer: answer,
                quickReplies: ["Info jurusan", "Info ekskul", "Berita terbaru"]
            });
        }
